...
def test_mass_mutation_flags_denied(flag):
    assert write("delete", "pod", flag, "app=web", "-n", "chaos") is Verdict.DENY
...
def test_dangerous_deletion_flags_denied(flag):
    assert write("delete", "pod", "web-abc", flag, "-n", "chaos") is Verdict.DENY


def test_selector_denied_even_when_it_looks_scoped():
    assert write("delete", "pod", "--selector=app=web", "-n", "chaos") is Verdict.DENY
...
def test_deletable_kinds(kind):
    assert write("delete", kind, "thing", "-n", "chaos") is Verdict.DESTRUCTIVE
...
def test_undeletable_kinds(kind):
    ...
    assert write("delete", kind, "thing", "-n", "chaos") is Verdict.DENY


def test_delete_namespace_denied_even_for_the_writable_one():
    assert write("delete", "namespace", "chaos") is Verdict.DENY
    assert write("delete", "ns", "chaos", "-n", "chaos") is Verdict.DENY


def test_delete_by_slash_form_is_parsed():
    assert write("delete", "pod/web-abc", "-n", "chaos") is Verdict.DESTRUCTIVE
    assert write("delete", "ns/chaos") is Verdict.DENY


def test_delete_requires_a_named_target():
    assert write("delete", "pod", "-n", "chaos") is Verdict.DENY
...
def test_denials_explain_themselves():
    ...
    d = classify(K_WRITE, {"args": ["delete", "namespace", "chaos"]})
...
def test_every_verdict_has_a_nonempty_reason():
    for tool, ti in [
        ...
        (K_WRITE, {"args": ["delete", "pod", "web", "-n", "chaos"]}),
